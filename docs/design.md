...
3. 本轮无新内容 → 只回复 `PASS`;
4. 认为任务完成/讨论收敛 → 发言后末行输出 `[DONE]`，作为交给 Scheduler 的完成候选；它不拥有结束权。

**终止条件(三重保险,防死循环)**:

| 条件 | 语义 |
|---|---|
| Scheduler 输出 `END` | Scheduler 确认任务结果可用、关键待办与异议均已关闭后的唯一业务完成判定 |
| 连续所有 Agent `PASS` | 无进展时的系统兜底，不作为任务验收 |
| 达到 `max_rounds` | 强制兜底 |
...
| **协议依赖模型自觉** | `PASS`/`[DONE]` 靠 prompt 约束,模型可能不遵守 | `[DONE]` 不具备结束权；即使完全不遵守也有 `max_rounds` 兜底 |
...
- ✅ Scheduler 可根据完整 transcript 判定收敛并结束;
- ✅ transcript 正确落盘。
...
2. **验收标准**:目前 Scheduler 通过自然语言任务与 transcript 判断完成。对关键流程，是否应支持在配置中声明结构化验收标准，以降低模型误判?
